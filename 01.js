(function(a, b) {
	...
	var c = a.document,
		...
		f = function() {
			...
			}), z = e.uaMatch(y), z.browser && (e.browser[z.browser] = !0, e.browser.version = z.version), e.browser.webkit && (e.browser.safari = !0), j.test(" ") && (k = /^[\s聽]+/, l = /[\s聽]+$/), h = e(c), c.addEventListener ? B = function() {
...
function ajaxInSameDomain(c, e, f, d, b, a) {
	...
	$.ajax({
		...
		success: function(g) {
			if (g.idsIntercepted) {
				var i = function() {
						ajaxInSameDomain(c, e, f, d, b, a)
					};
				if (g.policy == "GATEWAY" && g.status == "UNKNOWN") {
					probeAuthStatus(i, i, a);
					return
				}
				if (g.policy = "RESTRICTED") {
					if (g.status == "ANONYMOUS") {
						loginCallbackStack.push(i);
						popupLoginContainer(a);
						return
					} else {
						if (g.status == "UNKNOWN") {
							var h = function() {
...
function ajaxCrossDomain(c, e, d, b, a) {
	if (typeof a == "undefined") {
		if (typeof passport_config != "undefined") {
			a = passport_config
		} else {
	...
	$.ajax({
		url: c,
		data: e == null ? "crossDomainJsonpRequest=true" : e + "&crossDomainJsonpRequest=true",
		crossDomain: true,
		dataType: "jsonp",
		cache: false,
		error: b,
		success: function(f) {
			if (f.authStatusResponse) {
				var g = function() {
						ajaxCrossDomain(c, e, d, b, a)
					};
				if (f.hasLogin) {
					g()
				} else {
					loginCallbackStack.push(g);
					popupLoginContainer(a)
				}
				return
			}
...
if (!window.XMLHttpRequest) {
	$(window).scroll(function() {
		...
		var b = $(window).height();
		var c = $("#modalContainer").find("iframe").height();
		var d = Math.max(0, (b - c) / 2);
		$("#modalContainer").css({
			top: a + d
		})
	})
